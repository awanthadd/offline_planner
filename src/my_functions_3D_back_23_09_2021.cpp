...
std::cout<<"route.wpt_list.at(i+1).v.."<<route.wpt_list.at(i+1).v<<std::endl;
 ...
 // turn_struct cl_turn;
  if(abs(max_climb_ang)>abs(climb_angle)){ //WE WILL CALCULATE THE CLIMB WITH MAX CLIMB ANGLE
  // REPLACE THAT STRAIGHT IWTH THE CLIMB STRUCT...
...
std::cout<<" new_climb_ang.AAA:  "<<new_climb_ang<<std::endl;
  ...
  // else consider head change=0 and check for vertical climb...
{     
turn= generate_fly_by_turn_sub(route, turn_i,curvs,curve_wpt1,curve_wpt2,curve_wpt3, head_change,heading1, heading2);
...
{ // If head change ~=0 and climb angle~=0, a turn with no head change
 ...
 turn.end=turn.start; turn.points.push_back(turn.start); 
}
...
for (int i=0;i<plan_wps_XYZ.size();i++){ // i=0 or 1?...22/08/2021
  ...
  point_xyz pt1; pt1.x=plan_wps_XYZ.at(0).x; pt1.y=plan_wps_XYZ.at(0).y; pt1.z=plan_wps_XYZ.at(0).z;
...
mission_struct Generate_mission_from_LatLonAlt(const std::vector <point_LatLonAltVelFo>& wpsLatLonAltV, const mission_struct& mission){
 ...
 //updated_mission.items.push_back(Takeoff_item);
  ...
  for (int i=1; i < wpsLatLonAltV.size();i++){ // starting from the second element to avoid duplication of start point
      ...
      item1.frame=0;
      item1.params.push_back(0); item1.params.push_back(0); item1.params.push_back(0); item1.params.push_back(0);
      ...
      if(i==1) { // include a speed change command to navigate 1st waypont after take off 
        mission_item_struct item2; item2.command=138; item2.doJumpId=updated_mission.items.size() +1;
    ...
    if((i>1) &&(wpsLatLonAltV.at(i).Vel != wpsLatLonAltV.at(i-1).Vel ) ){ // include speed change wp
